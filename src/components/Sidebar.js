...
import { ChevronUp, ChevronDown, KeyRound, LogOut } from 'lucide-react';
          ...
          >
            ...
            {isDropdownOpen && (
              <div className="mt-2 py-1 border-t border-gray-100">
                        <button onClick={onResetPassword} className="w-full flex items-center px-3 py-2 text-sm text-gray-700 hover:bg-gray-100">
                  <KeyRound className="w-4 h-4 mr-2" /> Reset Password
                </button>
                        <button onClick={onLogout} className="w-full flex items-center px-3 py-2 text-sm text-gray-700 hover:bg-gray-100">
...
export default function Sidebar({ isSidebarOpen, setIsSidebarOpen, logout }) {
    ...
    const [user, setUser] = useState({ username: "John Doe", email: "john.doe@example.com" });
  ...
  const fetchCurrentUser = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (auth0User) {
          const userData = {
            username: auth0User.user_metadata?.username || auth0User.email.split('@')[0],
    ...
    const updatedMenu = [
        {
            ...
            alwaysOpen: true,
            children: [
                { name: "3D Floor Maps", path: "/dashboard", icon: (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 3v18h18" /><path d="M19 21V9L9 3v18" /><path d="M9 3L19 9" /></svg>) },
                { name: "Emergency Exits", path: "/security-view", icon: (<img width="20" height="20" src="https://img.icons8.com/ios-filled/50/logout-rounded.png" alt="logout-rounded"/>) },
                { name: "Air Quality", path: "/iaq", icon: (<img width="20" height="20" src="https://img.icons8.com/fluency-systems-regular/50/air-quality.png" alt="air-quality"/>) },
                { name: "Water Leakage", path: "/leakage", icon: (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2C12 2 6 10 6 14a6 6 0 0012 0c0-4-6-12-6-12z" /></svg>) },
            ],
        },
        {
            name: "History & Trends",
          ...
          children: [
            {
              ...
              path: "/editspaces",
              icon: (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9" /><path d="M16.5 3.5a2.121 2.121 0 113 3L5 21H2v-3L16.5 3.5z" /></svg>),
          },
          {
            name: "Edit Floor Plans",
            path: "/editor",
            icon: (<img width="20" height="20" src="https://img.icons8.com/metro/26/floor-plan.png" alt="floor-plan"/>),
        },  
          ],
      },
                    ...
                    ? [
                        {
                            name: "Manage Users",
                            path: "/users",
                            icon: (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="4" /><path d="M6 21v-2a6 6 0 0112 0v2" /></svg>),
                        },
                    ]
                    : []),
            ],
    ...
    if (searchQuery.trim() === "") {
        ...
        const filtered = menuItems
            .map((item) => {
                ...
                if (matchingChildren.length > 0 || item.name.toLowerCase().includes(searchQuery.toLowerCase())) {
...
const sendPasswordResetEmail = async () => {
  try {
      const response = await fetch("https://optimus-sandbox.us.auth0.com/dbconnections/change_password", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            K
                ...
                <ul>
                    {filteredItems.map((item, index) => (
                        <li key={index}>
                            <button className={`flex items-center font-semibold justify-between w-full px-4 py-2 text-left text-gray-700 hover:bg-gray-100 ${
                                openDropdown === item.name ? "bg-gray-100" : ""}`} onClick={() => setOpenDropdown(openDropdown === item.name ? "" : item.name)}>
                                <span>{item.name}</span>
                            ...
                            {openDropdown === item.name && (
                                <ul className="pl-6">
                                    {item.children.map((child, childIndex) => (
                                        <li key={childIndex}>
                                            <button className={`w-full rounded-md  flex px-4 py-2 text-left text-gray-700 ${child.disabled ? "text-gray-400 cursor-not-allowed" : "hover:bg-gray-100 w-72 mt-2"}`} onClick={() => handleNavigate(child.path)} disabled={child.disabled}>
             ...
             {isOverlayOpen && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                    <div className="bg-white p-4 rounded-lg shadow-lg text-center w-64">
                        <h2 className="text-[16px] font-semibold text-gray-900">Password Reset Sent</h2>

...
const HourlyZoneOccupancy = ({ selectedDate, selectedZone }) => {
  ...
  // Zone mapping for standardization (from the ZoneAnalytics component)
  const zoneMapping = {
    "South-zone": "Zone A",
    "South-Zone": "Zone A",
    "Central-zone": "Zone B",
  ...
  // Reverse mapping to identify original zone names from the standardized names
  const reverseZoneMapping = {};
  Object.keys(zoneMapping).forEach(originalName => {
    const standardizedName = zoneMapping[originalName];
    if (!reverseZoneMapping[standardizedName]) {
      reverseZoneMapping[standardizedName] = [];
    }
    reverseZoneMapping[standardizedName].push(originalName);
  });
  ...
  const processHourlyData = (data) => {
    ...
    // Get all possible original zone names for the selected standardized zone
    const originalZoneNames = reverseZoneMapping[selectedZone] || [];
    
    // Filter data for the selected zone (including all variations)
    const filteredData = data.filter(item => {
      // Skip Relocated zone as requested
      if (item.zone_name.toLowerCase() === "relocated") return false;
      
      // If it's already using the standardized name
      if (item.zone_name === selectedZone) return true;
      
      // Check if this zone's original name matches any of the mapped names
      return originalZoneNames.some(zoneName => 
        item.zone_name === zoneName || 
        item.zone_name.toLowerCase() === zoneName.toLowerCase()
      );
    });
  ...
  // Fetch data when selected date or zone changes
  useEffect(() => {
    fetchHourlyData();
          ...
          <div className="flex items-center mb-2">
            ...
            {hours.map((hour, index) => (
              <div key={hour} className="flex-1 text-center">
                <div 
                  className={`mx-1 py-4 px-2 rounded-md ${
